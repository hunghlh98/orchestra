...
3. End turn. No agent spawn. No bootstrap. No persistence.
...
- **User typed `/orchestra help`**: NOT S1. `$1 = "help"` triggers S8/S9 router (unknown subcommand). Router's Q1 reads "help" as restate-intent seed → 3× `AskUserQuestion` rather than usage block. If user wants usage, they invoke `/orchestra` bare.
