...
Standard `spawn`/`read`/`pause`/`shutdown` lines per the dispatcher's Status output spec apply for each subsequent agent in the resumed flow.

## Return contract

The skill returns control to the dispatcher with:
- `terminal_state ∈ {success, deadlock, escalated, "(continued)"}`. The "(continued)" value indicates the resume point was found and dispatcher should continue per Step 5 of the smart router.
