...
- **README rewrite** — restructured against the per-plugin reference style: counts in section headers (`## Skills (10)`, `## Agents (9)`, `## Hooks (7)`, etc.), one table per component kind, env-var opt-out registry inlined, dev-surface markdown links removed. Fixes drift the 2026-05-14 audit caught (stale counts, phantom `project-discovery` skill, fictional subcommand names `ship`/`report`/`resume`/`help` that no longer exist in `commands/orchestra.md`, links into `manifests/` and `scripts/`).
...
- **`@test` → `@test-author` + `@test-runner` split.** The two-stage tester becomes two distinct agents. `@test-author` is spec-bound (`tools: Read, Write, Edit, MultiEdit, Glob, Grep, Skill` — no Bash); `@test-runner` is impl-aware (same allowlist plus Bash for `mvn test` / `npm test` / `pytest`). Anti-confirmation-bias is now structural at the frontmatter layer instead of honor-system Stage-1/Stage-2 prose. All chain-artifact references (lead/architect/backend/frontend agents, `commands/orchestra.md`, `schemas/routing-taxonomy.md`, `schemas/pipeline-artifact.schema.md`, TSR template, `qa-test-planner` / `write-contract` / `task-breakdown` / `java-development` skills, `scripts/tests/agents.test.js`) updated to name the two-agent split. Agent count 8 → 9.
- **`@evaluator` + `@reviewer` move to `tools:` allowlist.** Both agents previously used `disallowedTools:` denylist; now both declare `tools:` explicitly (`@evaluator`: `Read, Write, Glob, Grep, Skill`; `@reviewer`: same plus `Bash` for read-only static analysis). Closes the denylist gap (new tool kinds were default-allowed); allowlist is default-deny. `scripts/tests/agents.test.js` Check 4 simplifies — the `@test`-special-case branch ("must declare neither") disappears.
