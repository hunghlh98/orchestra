...
Patch release. Relocates the consumer orchestra-rules block out of `CLAUDE.md` into an imported file.

### Changed

- **`claude_md` externalizes the orchestra rules to `.orchestra/claude-md/orchestra.md` + an `@`-import line (`mcp-servers/orchestra-utils.js`, `hooks/lib/bootstrap-consumer-claude-md.js`).** The tool now writes the rules body to `<ctx>/.orchestra/claude-md/orchestra.md` (with a `generated by orchestra` banner) and adds a single `@.orchestra/claude-md/orchestra.md` import line to `<ctx>/CLAUDE.md`, replacing the inline `<!-- orchestra:start/end -->` marker splice. The fixed import line is its own idempotency anchor, so the marker block and the `section()` / `spliceClaudeMd()` / `freshClaudeMd()` helpers are removed; the symlink guard now covers both write targets and the tool return adds `rules_path`. Keeps the consumer `CLAUDE.md` to a single line and isolates orchestra rule regeneration from hand-written consumer content.
