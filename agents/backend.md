...
5. Write code. Match project conventions (formatter, imports, package layout) AND Clean Architecture: business logic in `use-cases/`, framework in `interface-adapters/`, no inward leakage. Names reveal intent; short functions; null avoided.
   - **Cross-process boundary instrumentation.** Every outbound HTTP call, every Kafka publish, every Kafka consumer method, every DB error path emits INFO on receipt AND outcome. HTTP: route + status + latency. Kafka consume: `topic` + `partition` + `offset` on receipt, transition or rejection on outcome. Silent success is indistinguishable from "no traffic" during bring-up — `@reviewer` flags missing instrumentation as Major per `skills/code-review`.
   - **JPA / Jackson / Kafka / Resilience4j / persistence-port discipline.** Apply `skills/java-development/references/jpa-write-conventions.md`: `mappedBy` references a `@ManyToOne`; read paths that dereference lazy collections carry `@EntityGraph` or `@Transactional(readOnly=true)`; inbound event DTOs carry `@JsonIgnoreProperties(ignoreUnknown=true)`; cache-round-trip immutables carry `@JsonCreator` + `@JsonProperty`; `@KafkaListener` topic defaults match the publisher's literal value; `@Retry` instances declare explicit `retry-exceptions`; repository ports expose intent-specific methods (`save` = INSERT only, `applyTransition` = UPDATE).
