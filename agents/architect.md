...
3. Set frontmatter counts (`owned_table_count`, `invariant_count`, `sub_capability_count`) to the row counts, then flip `status: locked`. Hand back to `@lead`; downstream agents (`@product`, `@lead`, implementers) read CSD as read-only reference for the rest of the run.
