...
`<feature-id>-openapi.yaml` (locked) + `-PRD.md` + `-FRS.md` + `-TDD.md` + `<feature-id>-TASKS.md` (your `owner: @test-author` rows).

NOT `<context_path>/services/<service_name>/src/main/**` (honor-system block per spawn-prompt invariant above).
...
- After Write: `sections.S-TEST-001.status: in_progress`.

Black-box tests carry NO chain-artifact cites in source (`pre-write-check.js` Gate-D rejects). Test names = domain-only ("rejects empty username" not "validates FR-3, AC-2").
