...
- **Missing client contract** — under brownfield `scope_level ∈ {container, service}` runs where CSD exists: for every `<context_path>/docs/<service_name>/<service_name>-CSD.md` `S-CONTRACT-001` row with `direction: outbound-http` that the feature under review exercises (callsite present in the diff), there MUST be a matching `<context_path>/docs/<service_name>/<feature-id>/<feature-id>-clientapi.yaml` documenting the upstream contract this feature requires (one file covers all outbound HTTP deps for the feature). Missing or empty `clientapi.yaml` while outbound HTTP callsites land in the diff → REQUEST_CHANGES with `missing-client-contract: <upstream-path> at <file:line>`. Greenfield runs and features with no outbound HTTP callsites are exempt.
