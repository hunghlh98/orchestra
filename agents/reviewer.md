...
- **Diagram-allowlist violation** — `.puml` files with forbidden prefixes (`AD-*`, `SAGA-*`, `SD-*`, `ERD-*`, `C2-*`, `C3-*`). Allowed filenames by scope: system-level under `docs/diagrams/` → `c4-context.puml`, `c4-container.puml`, `erd-logical.puml`, `sequence-inter-<flow>.puml`. Service-level under `docs/<service_name>/diagrams/` → `c4-component.puml`, `c4-code.puml` (one of each per service; service-level L3 + L4 do NOT have per-feature copies). Per-feature under `docs/<service_name>/<feature-id>/diagrams/` → `<feature-id>-c4-context.puml`, `<feature-id>-c4-container.puml` (highlighted copies of system L1+L2 — NO `<feature-id>-c4-component-*` or `<feature-id>-c4-code-*`), `<feature-id>-seq-<usecase>.puml`, `<feature-id>-state-technical.puml`, `<feature-id>-frs-usecase.puml`, `<feature-id>-state-business.puml`, `<feature-id>-erd-physical.puml`. Per `skills/c4-architecture/SKILL.md` "File layout" + `skills/plantuml/SKILL.md` Step 2a.
...
- **Missing client contract** — for every outbound HTTP callsite (`RestTemplate` / `WebClient` / Feign client interfaces / `RestClient` / `HttpClient`) present in the feature's implementation diff, there MUST be a matching `<context_path>/docs/<service_name>/<feature-id>/<feature-id>-clientapi.yaml` operation documenting the upstream contract this feature requires (one file covers all outbound HTTP deps for the feature). Missing or empty `clientapi.yaml` while outbound HTTP callsites land in the diff → REQUEST_CHANGES with `missing-client-contract: <upstream-path> at <file:line>`. Features with no outbound HTTP callsites are exempt.
- **Use-case diagram missing end-user actor** — `<context_path>/docs/<service_name>/<feature-id>/diagrams/<feature-id>-frs-usecase.puml` MUST declare at least one `actor` matching an end-user persona from the feature's PRD `S-STAKEHOLDERS-001` and connecting to ≥1 use case. Scan: extract `actor "<label>"` declarations from the `.puml`; cross-reference labels against PRD `S-STAKEHOLDERS-001` end-user rows (operators, back-office, internal services, BFFs are NOT end users — they're proximate callers and may appear in addition to the end-user actor, never instead of). Zero matching end-user actors → REQUEST_CHANGES with `usecase-missing-end-user: <feature-id>-frs-usecase.puml does not include any actor from PRD S-STAKEHOLDERS-001 end-user rows`. Per `agents/product.md` step 6.
...
- **Feature attribution in CSD body** — any `<context_path>/docs/<service_name>/<service_name>-CSD.md` body row outside `S-SUB-CAPABILITIES-001` containing a feature-id reference (e.g., `#001-order-placement`, `(feature 002)`, `added by feature N`, `introduced by <feature-id>`). The five service-grain anchors describe the service's current consolidated state — see `schemas/csd.schema.md` "Body grammar: living service-grain state". Push the row back to the feature's TDD / FRS / openapi, or rewrite without the attribution if the row genuinely describes service-grain state.
...
- **Unworthy ADR** — proposed ADR (frontmatter `status: proposed`) whose body does not show evidence for all three ADR-worthiness gates per `agents/architect.md` "ADR-worthiness gates". Scan: (gate 1) ≥2 named alternatives in `S-ALTERNATIVES-001` rows with non-trivial pros/cons — a row with empty pros/cons or "no realistic alternative" copy fails; (gate 2) `S-CONTEXT-001` or `S-CONSEQUENCES-001` names ≥2 distinct files / components / services affected by the choice; (gate 3) `S-CONSEQUENCES-001` cites at least one of: external-contract impact, data-shape migration, cross-team sign-off, production-behavior change. Missing any of the three signals → REQUEST_CHANGES with `unworthy-adr: missing gate-<N> evidence`. Recommend the fall-back artifact in the finding body (CSD `S-INVARIANTS-001` row for half-implemented or accidental shapes, inline PRD/FRS/TDD body decision for local conventions, `AskUserQuestion`-resolved choice for small forks). Brownfield reverse-doc DIV rows that arrived as ADR proposals are always unworthy — they belong on Path A or Path B of `agents/architect.md` "DIV resolution paths".
...
For ADR-gap flagging (impl diff carries an undocumented decision passing all three ADR-worthiness gates): `<context_path>/.orchestra/<service_name>/pipeline/<feature-id>/<feature-id>-ESCALATE-ADR-<NNNN>.md` with `triggered_by_agent: "@reviewer"`, naming the undocumented decision and proposing a slug for `@architect` to open the ADR.
