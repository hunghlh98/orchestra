...
- **Allowed-set violation** — any artifact written outside its owning agent's allowed-set (`@product` writing `<feature-id>-spec.md`, `@architect` writing `<feature-id>-architecture.md`, `@lead` writing `CONTRACT-NNN-*.md`). Each agent's allowed-set in its `## Rules → ### Allowed surface`.
...
- **Scope-content mismatch on workspace SAD** — `workspace_kind: multi-repo` (read from `<context_path>/.orchestra/system.yaml`) AND `<context_path>/docs/SAD.md` `S-CONTAINERS-001` enumerates <2 service-Container rows. Workspace SAD declares the platform as System under design; a single Container row + N `System_Ext` siblings is a service-scope L1/L2 wearing a workspace label. Same check applies to `<context_path>/docs/diagrams/c4-container.puml` (workspace L2): the `System_Boundary(...)` must enclose ≥2 `Container(...)` entries that correspond to services in `<context_path>/CLAUDE.md` Service Topology — services listed in the topology table rendered as `System_Ext` are an automatic mismatch. Per `agents/architect.md` `### C4 scope continuity` + `hooks/scripts/pre-write-check.js` Gate-E (which catches the SAD-side floor at write time; the puml-side semantic check — "service-topology entry rendered as System_Ext" — is reviewer's). Finding shape: `scope-content-mismatch: <SAD path> S-CONTAINERS-001 lists <N> Containers, expected ≥<topology count from CLAUDE.md>`.
...
| `tools` | `Read, Write, Glob, Grep, Bash, Skill` | Bash for read-only static analysis (`eslint`, `mvn checkstyle`); Edit/MultiEdit denied (no diff patching). |
| `color` | `red` | Verification tier visual tag (reviewer). |
...
- `clean-architecture` — score `S-REVIEW-001` against 6-principle rubric (Dependency Rule / Entities & Use Cases / Adapters & Frameworks / Component Principles / SOLID / Boundaries). Goal: 10/10. Below 8 → Major finding citing principle + boundary violation. Critical when business rules import framework directly.
