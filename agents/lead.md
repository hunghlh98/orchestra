...
7. **Author openapi.yaml.** Invoke `write-contract`. Each operation's `description:` carries criteria with weights (sum to 100); mark security/data-loss criteria `critical: true`. Each criterion cites the FRS `S-AC-001` `AC-NNN` row it operationalizes (e.g., `criterion ... (AC-003)`); test probes downstream pin to those AC ids so TSR `S-TEST-001` rows trace back through FRS → CSD/SAD parent rule. Top-of-file `# orchestra:` comment block holds artifact frontmatter (id, type, status, sections, etc.). Flip `status: locked` only when criteria are complete and probable.
