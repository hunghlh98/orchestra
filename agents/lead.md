...
4. **Persistence shape priority.** When `S-DATA-001` enumerates entities, read sources in priority order: (i) `src/main/resources/db/migration/V*.sql` (Flyway) or `src/main/resources/db/changelog/*.xml` (Liquibase) when present — canonical schema; (ii) entity classes (`@Entity` / equivalent) — fallback when migrations absent. `S-DATA-001` notes the derivation: `derived from: flyway | liquibase | entities | ddl-auto`. `ddl-auto` value triggers `DEFECT-ddl-auto-not-versioned.md` per `S-CONFIG-001` row inventory (above).
5. **No fan-out during reverse-pass.** Implementer fan-out is forward-chain only. Hand back on TDD + openapi lock.
