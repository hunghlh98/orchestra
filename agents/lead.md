...
1. Read `local.yaml.discovery` + the locked intel artifacts at `<context_path>/.orchestra/<service_name>/source-intel/backend-intel.md` (always) and `frontend-intel.md` (UI projects). Read the reverse-doc `<feature-id>-PRD.md` + `<feature-id>-FRS.md` already authored by `@product`. Under depth=full, also read reverse-doc SAD + accepted ADRs. Do NOT direct-walk source — the intel artifacts are the comprehension layer.
