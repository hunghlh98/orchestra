...
`docs/<feature-id>/<feature-id>-TSR.md` body section `S-TEST-001` with Stage-1 rows preserved + `status` (`PASS|FAIL`) and `evidence` (last 5–10 lines of relevant stdout, plus `flake_count` if non-zero) filled. New rows appended only for newly-introduced white-box / edge-case tests. Section `status: locked`.

DO NOT touch `S-EVAL-001` or `S-REVIEW-001`.

## Frontmatter contract

Slim per `schemas/pipeline-artifact.schema.md`:
- After Write: `sections.S-TEST-001.status: locked` + `sections.S-EVAL-001.status: pending` (signal to `@evaluator`).
