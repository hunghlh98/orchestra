...
14. **Author run-plan.md + approval gate.** Brownfield only at depth ≥ medium: BEFORE spawning `@lead`, fan out `@backend` (always) + `@frontend` (UI projects) + `@test` (when tests exist) in a single Agent-tool-call message with prompt-tag `task: source-explore`. Wait for all `<stack>-intel.md` artifacts to flip `status: locked`. Then spawn `@lead` with prompt-tag `task: run-plan-author` (see "Run-plan author" section below). Lead writes `<context_path>/.orchestra/<service_name>/run-plan.md` against `schemas/run-plan.schema.md`. **Brownfield:** lead reads locked intel artifacts inside `EnterPlanMode` (validation pass over `S-FEATURE-CANDIDATES-001`, not cold source-walk) + `ExitPlanMode` for native plan-approval (approval happens inside lead's turn). **Greenfield:** lead writes directly; dispatcher then runs `AskUserQuestion(approve|revise)`. On approval (either branch), dispatcher writes `auto_mode: true` + `run_plan_status: approved` to `local.yaml`. On rejection, dispatcher writes `run_plan_status: revision_requested`, collects revision notes via `AskUserQuestion`, re-spawns lead. Max 3 cycles; cycle 4 → write `<context_path>/.orchestra/<service_name>/pipeline/run-plan-ESCALATE.md`.
15. **Bootstrap CLAUDE.md.** Run `node ${CLAUDE_PLUGIN_ROOT}/hooks/lib/bootstrap-consumer-claude-md.js <cwd>` via Bash. Idempotent: creates `<cwd>/CLAUDE.md` if missing; otherwise splices the orchestra section between `<!-- orchestra:start -->` / `<!-- orchestra:end -->` markers (preserves user content). No-op when current.
...
2. **Brownfield bootstrap branch (one-shot).** If `local.yaml.mode == brownfield` AND `local.yaml.bootstrap != completed`, fan out reverse-doc spawns per the elected `depth` BEFORE entering the forward chain. Each major feature gets a freshly-minted `<feature-id>` (= `<NNN>-<slug>`; see "Feature-id minting" below).
   - **Step 2a — source-explore fan-out (depth ∈ {medium, full}).** Before any `mode: reverse-doc` spawn, fan out `@backend` (always) and `@frontend` (when inventory shows a UI layer) and `@test` (when existing tests detected) in a single Agent-tool-call message, each with prompt-tag `task: source-explore`. Each implementer writes `<context_path>/.orchestra/<service_name>/source-intel/<stack>-intel.md` (read-only mode; no source edits, no Bash). Wait for all locked. Skip Step 2a entirely on `depth: light` — Light reverse-doc only needs PRD, which `@product` can author from inventory alone.
   - **Step 2b — feature-id minting (depth ∈ {medium, full}).** Read `backend-intel.md` `S-FEATURE-CANDIDATES-001` (and `frontend-intel.md` when present). Per "Feature-id minting" below, mint one `<feature-id>` per candidate the dispatcher accepts.
   - **Step 2c — reverse-doc fan-out per depth:**
     - `light` — spawn `@product` per major feature with prompt-tag `mode: reverse-doc`. Output: `<feature-id>-PRD.md` per feature. (No intel artifacts; `@product` reads inventory + scope_hints only.)
     - `medium` — spawn `@product` (PRD + FRS) and `@lead` (TDD) per major feature with prompt-tag `mode: reverse-doc`. Both read locked intel artifacts (already produced in Step 2a). Author paths run sequentially per feature (PRD → FRS → TDD); features run in parallel.
     - `full` — first spawn `@architect` (project-level SAD + accepted ADRs based on visible-in-source decisions) with prompt-tag `mode: reverse-doc`. Then per major feature: `@product` (PRD + FRS) → `@lead` (TDD + openapi). All read locked intel artifacts.
   After all reverse-doc spawns idle, parent flips `local.yaml.bootstrap: completed` and proceeds to Step 3. On subsequent `/orchestra` runs, this step is a no-op (greenfield-equivalent forward chain). Intel artifacts cache one-shot per `pipeline_id`; re-run Step 2a only when `local.yaml.source_lock.read_paths` changes.
