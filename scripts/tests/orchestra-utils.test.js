...
//   claude_md create / append / import-idempotency / rules-drift / symlink reject
...
const CLAUDE_MD_IMPORT_LINE = "@.orchestra/claude-md/orchestra.md";
const CLAUDE_MD_RULES_REL = join(".orchestra", "claude-md", "orchestra.md");
const CLAUDE_MD_BODY_PHRASE = "No chain-artifact cites in business code"; // unique line from the template
withTmp(tmp => {
  const rulesPath = join(tmp, CLAUDE_MD_RULES_REL);

  // Fresh create: CLAUDE.md holds the import line; the body goes to the rules file.
  ...
  check(created.includes(CLAUDE_MD_IMPORT_LINE), "fresh: CLAUDE.md contains @-import line");
  check(!created.includes(CLAUDE_MD_BODY_PHRASE), "fresh: rules body NOT inlined into CLAUDE.md");
  check(existsSync(rulesPath), "fresh: rules file written");
  const rules = readFileSync(rulesPath, "utf8");
  check(rules.includes("generated by orchestra"), "fresh: rules file carries generated-by banner");
  check(rules.includes(CLAUDE_MD_BODY_PHRASE), "fresh: rules file holds template body");

  // No-op on re-run (import present + rules file current)
  const b = claudeMdImpl({ context_path: "." });
  check(b.action === "unchanged", `re-run: action='unchanged' (got ${b.action})`);

  // Append the import into a pre-existing CLAUDE.md that lacks it
  ...
  check(appended.includes(CLAUDE_MD_IMPORT_LINE), "append: adds @-import line");

  // Rules-file drift: import present but body stale → rewrite, action 'updated'
  writeFileSync(rulesPath, "stale body\n");
  const d = claudeMdImpl({ context_path: "." });
  check(d.action === "updated", `drift: action='updated' (got ${d.action})`);
  const refreshed = readFileSync(rulesPath, "utf8");
  check(!refreshed.includes("stale body"), "drift: stale rules body replaced");
  check(refreshed.includes(CLAUDE_MD_BODY_PHRASE), "drift: rules file restored from template");
});

// ---------- claude_md: symlink reject (CLAUDE.md) ----------
console.log("claude_md symlink reject (CLAUDE.md):");
withTmp(tmp => {
...
// ---------- claude_md: symlink reject (rules file) ----------
console.log("claude_md symlink reject (rules file):");
withTmp(tmp => {
  mkdirSync(join(tmp, ".orchestra", "claude-md"), { recursive: true });
  symlinkSync("/tmp/decoy-orchestra-md", join(tmp, ".orchestra", "claude-md", "orchestra.md"));
  let rejected = false;
  try { claudeMdImpl({ context_path: "." }); }
  catch (e) { rejected = /symlink|refusing/.test(e.message); }
  check(rejected, "refuses to operate on symlinked rules file");
});
